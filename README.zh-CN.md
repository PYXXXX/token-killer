...
[在线体验](https://burn.bilirec.com) · [部署自己的 Token Killer](docs/deployment.zh-CN.md) · [API 参考](docs/api.zh-CN.md)
...
排行榜使用随机生成的编号识别参与者，不提供自定义昵称。默认开启的自动赛区选择会先使用 `/api/geo/assertion` 接口签发网络出口地区凭证，失败后再回退到排行榜边缘节点；用户也可以关闭自动选择，手动指定国家、一级行政区和城市。所有国家在能够取得相应字段时都支持地区与城市排行榜；香港特别行政区、澳门特别行政区和台湾省仍归入“中国”目录，其中台湾省支持城市排行榜，香港和澳门仅支持省级排行。应用与排行榜数据库都不会保存原始 IP。
