...
若这台 VPS 还承担大陆赛区探测，请参阅[部署指南](../../docs/deployment.zh-CN.md#可选优先识别大陆直连地区)。Node 服务可以用本地 GeoLite2 City 数据库查询 Caddy 传入的来源 IP；只有上游可信代理会清除访客输入并注入经过校验的 `X-Geo-*` Header 时，才可改用 `TRUST_GEO_HEADERS=true`。
